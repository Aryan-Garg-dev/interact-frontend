            ...
            <div className="w-full font-primary flex gap-1 dark:text-white py-2 border-primary_btn dark:border-dark_primary_btn border-b-[1px] max-md:px-4 max-md:py-4">
              <div className="w-[15%] h-full">
                <div className="rounded-full">
                  <Image
                    ...
                    height={50}
                    alt={'User Pic'}
                    src={`${USER_PROFILE_PIC_URL}/${message.announcement.organization?.user.profilePic}`}
              ...
              <div className="w-[85%] flex flex-col gap-3">
                <div className="w-full h-fit flex justify-between">
                  <div className="font-medium">{message.announcement.organization?.user.username}</div>
                  <div className="flex gap-2 font-light text-xs">
                    {getDisplayTime(message.announcement.createdAt, false)}
                  </div>
                </div>
                <div className="w-full font-medium">{message.announcement.title}</div>
                <div className="w-full text-sm whitespace-pre-wrap overflow-clip mb-2">
